(() => {
  var e = {};
  ((e.id = 646),
    (e.ids = [646]),
    (e.modules = {
      7849: (e) => {
        "use strict";
        e.exports = require("next/dist/client/components/action-async-storage.external");
      },
      2934: (e) => {
        "use strict";
        e.exports = require("next/dist/client/components/action-async-storage.external.js");
      },
      5403: (e) => {
        "use strict";
        e.exports = require("next/dist/client/components/request-async-storage.external");
      },
      4580: (e) => {
        "use strict";
        e.exports = require("next/dist/client/components/request-async-storage.external.js");
      },
      4749: (e) => {
        "use strict";
        e.exports = require("next/dist/client/components/static-generation-async-storage.external");
      },
      5869: (e) => {
        "use strict";
        e.exports = require("next/dist/client/components/static-generation-async-storage.external.js");
      },
      399: (e) => {
        "use strict";
        e.exports = require("next/dist/compiled/next-server/app-page.runtime.prod.js");
      },
      9491: (e) => {
        "use strict";
        e.exports = require("assert");
      },
      4300: (e) => {
        "use strict";
        e.exports = require("buffer");
      },
      2081: (e) => {
        "use strict";
        e.exports = require("child_process");
      },
      6113: (e) => {
        "use strict";
        e.exports = require("crypto");
      },
      2361: (e) => {
        "use strict";
        e.exports = require("events");
      },
      7147: (e) => {
        "use strict";
        e.exports = require("fs");
      },
      3685: (e) => {
        "use strict";
        e.exports = require("http");
      },
      5158: (e) => {
        "use strict";
        e.exports = require("http2");
      },
      5687: (e) => {
        "use strict";
        e.exports = require("https");
      },
      1808: (e) => {
        "use strict";
        e.exports = require("net");
      },
      2037: (e) => {
        "use strict";
        e.exports = require("os");
      },
      1017: (e) => {
        "use strict";
        e.exports = require("path");
      },
      2781: (e) => {
        "use strict";
        e.exports = require("stream");
      },
      4404: (e) => {
        "use strict";
        e.exports = require("tls");
      },
      6224: (e) => {
        "use strict";
        e.exports = require("tty");
      },
      7310: (e) => {
        "use strict";
        e.exports = require("url");
      },
      3837: (e) => {
        "use strict";
        e.exports = require("util");
      },
      9796: (e) => {
        "use strict";
        e.exports = require("zlib");
      },
      4173: (e, s, r) => {
        "use strict";
        (r.r(s),
          r.d(s, {
            GlobalError: () => n.a,
            __next_app__: () => u,
            originalPathname: () => p,
            pages: () => l,
            routeModule: () => x,
            tree: () => c,
          }),
          r(1885),
          r(3321),
          r(4481),
          r(1799),
          r(6083),
          r(9644),
          r(5866));
        var t = r(3191),
          a = r(8716),
          o = r(7922),
          n = r.n(o),
          i = r(5231),
          d = {};
        for (let e in i)
          0 >
            [
              "default",
              "tree",
              "pages",
              "GlobalError",
              "originalPathname",
              "__next_app__",
              "routeModule",
            ].indexOf(e) && (d[e] = () => i[e]);
        r.d(s, d);
        let c = [
            "",
            {
              children: [
                "dashboard",
                {
                  children: [
                    "sacco",
                    {
                      children: [
                        "analytics",
                        {
                          children: [
                            "__PAGE__",
                            {},
                            {
                              page: [
                                () => Promise.resolve().then(r.bind(r, 1885)),
                                "C:\\Users\\wesley\\Desktop\\Radaa\\radaa-frontend\\app\\dashboard\\sacco\\analytics\\page.tsx",
                              ],
                            },
                          ],
                        },
                        {},
                      ],
                    },
                    {},
                  ],
                },
                {
                  layout: [
                    () => Promise.resolve().then(r.bind(r, 3321)),
                    "C:\\Users\\wesley\\Desktop\\Radaa\\radaa-frontend\\app\\dashboard\\layout.tsx",
                  ],
                  loading: [
                    () => Promise.resolve().then(r.bind(r, 4481)),
                    "C:\\Users\\wesley\\Desktop\\Radaa\\radaa-frontend\\app\\dashboard\\loading.tsx",
                  ],
                },
              ],
            },
            {
              layout: [
                () => Promise.resolve().then(r.bind(r, 1799)),
                "C:\\Users\\wesley\\Desktop\\Radaa\\radaa-frontend\\app\\layout.tsx",
              ],
              error: [
                () => Promise.resolve().then(r.bind(r, 6083)),
                "C:\\Users\\wesley\\Desktop\\Radaa\\radaa-frontend\\app\\error.tsx",
              ],
              loading: [
                () => Promise.resolve().then(r.bind(r, 9644)),
                "C:\\Users\\wesley\\Desktop\\Radaa\\radaa-frontend\\app\\loading.tsx",
              ],
              "not-found": [
                () => Promise.resolve().then(r.t.bind(r, 5866, 23)),
                "next/dist/client/components/not-found-error",
              ],
            },
          ],
          l = [
            "C:\\Users\\wesley\\Desktop\\Radaa\\radaa-frontend\\app\\dashboard\\sacco\\analytics\\page.tsx",
          ],
          p = "/dashboard/sacco/analytics/page",
          u = { require: r, loadChunk: () => Promise.resolve() },
          x = new t.AppPageRouteModule({
            definition: {
              kind: a.x.APP_PAGE,
              page: "/dashboard/sacco/analytics/page",
              pathname: "/dashboard/sacco/analytics",
              bundlePath: "",
              filename: "",
              appPaths: [],
            },
            userland: { loaderTree: c },
          });
      },
      1234: (e, s, r) => {
        Promise.resolve().then(r.bind(r, 4138));
      },
      4138: (e, s, r) => {
        "use strict";
        (r.r(s), r.d(s, { default: () => a }));
        var t = r(326);
        function a() {
          return (0, t.jsxs)("div", {
            className: "space-y-4",
            children: [
              (0, t.jsxs)("header", {
                className: "space-y-1",
                children: [
                  t.jsx("h1", {
                    className: "text-2xl font-semibold tracking-tight",
                    children: "SACCO analytics",
                  }),
                  t.jsx("p", {
                    className: "text-xs text-slate-300",
                    children:
                      "Visualise revenue, occupancy, and route performance for your SACCO.",
                  }),
                ],
              }),
              (0, t.jsxs)("section", {
                className:
                  "space-y-3 rounded-xl border border-slate-800 bg-slate-900/80 p-4 text-xs",
                children: [
                  (0, t.jsxs)("div", {
                    className: "space-y-2",
                    children: [
                      t.jsx("div", {
                        className:
                          "h-4 w-32 animate-pulse rounded bg-slate-800/80",
                      }),
                      t.jsx("div", {
                        className:
                          "h-32 animate-pulse rounded-lg bg-slate-800/80",
                      }),
                    ],
                  }),
                  t.jsx("p", {
                    className: "text-[11px] text-slate-300",
                    children: "Coming soon — backend integration pending.",
                  }),
                ],
              }),
            ],
          });
        }
      },
      1885: (e, s, r) => {
        "use strict";
        (r.r(s),
          r.d(s, { $$typeof: () => n, __esModule: () => o, default: () => i }));
        var t = r(8570);
        let a = (0, t.createProxy)(
            String.raw`C:\Users\wesley\Desktop\Radaa\radaa-frontend\app\dashboard\sacco\analytics\page.tsx`,
          ),
          { __esModule: o, $$typeof: n } = a;
        a.default;
        let i = (0, t.createProxy)(
          String.raw`C:\Users\wesley\Desktop\Radaa\radaa-frontend\app\dashboard\sacco\analytics\page.tsx#default`,
        );
      },
    }));
  var s = require("../../../../webpack-runtime.js");
  s.C(e);
  var r = (e) => s((s.s = e)),
    t = s.X(0, [489, 496, 462, 754], () => r(4173));
  module.exports = t;
})();
